...
class SinaDataSource(BaseDataSource):
    ...
    def fetch_stock_quote(self, ts_code: str) -> StockQuote | None:
        ...
        try:
            ...
            r = requests.get(mkt_url, params=params, headers=self.HEADERS, timeout=15)
            ...
            if item:
                ...
                return StockQuote(
                    ...
                    amount=float(item.get("amount", 0)),  # 市场中心API返回的amount已是元
    ...
    def fetch_daily_prices(
        ...
        Sina K线 API 仅返回 OHLCV，不返回成交额和换手率。
        成交额 = vol × 均价; 换手率 = vol / 流通股本。
        """
        import json
        ...
        try:
            ...
            # 查询流通股本用于计算换手率
            float_share_g = self._get_float_share(ts_code)
            ...
            for point in raw:
                ...
                if d < start_date.replace("-", "") or d > end_date.replace("-", ""):
                    continue
                o = float(point.get("open", 0))
                h = float(point.get("high", 0))
                l = float(point.get("low", 0))
                c = float(point.get("close", 0))
                v = float(point.get("volume", 0))

                # 成交额 = 成交量(股) × 均价
                avg_price = (o + h + l + c) / 4 if (o + h + l + c) > 0 else c
                amount = v * avg_price

                # 换手率 = 成交量(股) / 流通股本(股) × 100%
                turnover_rate = None
                if float_share_g and float_share_g > 0:
                    turnover_rate = round(v / float_share_g * 100, 4)
    ...
    @staticmethod
    def _get_float_share(ts_code: str) -> float | None:
        """从数据库获取流通股本（股）。

        数据库中 float_share 单位为万股，返回值为股。
        """
        try:
            ...
            try:
                ...
                if stock and stock.float_share:
                    return float(stock.float_share) * 10000  # 万股 → 股
                return None
            finally:
                db.close()
        except Exception:
